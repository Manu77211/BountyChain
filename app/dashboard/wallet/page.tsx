  ...
  return (
    <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="p-5">
          <p className="text-sm text-[#4b4b4b]">Released</p>
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button asChild variant="secondary">
            <Link href={user?.role === "FREELANCER" ? "/dashboard/wallet/withdraw" : "/dashboard/wallet/add-funds"}>
              {user?.role === "FREELANCER" ? "Withdraw" : "Add Funds"}
            </Link>
          </Button>
        </div>
        ...
        <div className="mt-4 space-y-2">
              ...
              return (
                <div key={`${project.id}-${milestone.id}`} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#121212] bg-[#f5f5f5] p-3">
                  <div>
                    ...
                    <p className="text-xs text-[#4b4b4b]">{new Date(milestone.updatedAt ?? Date.now()).toLocaleString()}</p>
                  </div>
          ...
          {projects.length === 0 ? <p className="text-sm text-[#4b4b4b]">No transactions yet.</p> : null}
        </div>
      </Card>
    </motion.section>
  );
