...
export default function DashboardProfilePage() {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {loading ? <p className="text-[#4b4b4b]">Loading profile...</p> : null}
        ...
        <Card>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-[#4b4b4b]">{profile.email}</p>
            </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-[#121212] bg-[#f5f5f5] p-4">
              <p className="text-xs text-[#4b4b4b]">Rating</p>
              <p className="mt-1 text-2xl font-semibold">{Number(profile.rating ?? 0).toFixed(1)}</p>
            </div>
            <div className="rounded-xl border border-[#121212] bg-[#f5f5f5] p-4">
              <p className="text-xs text-[#4b4b4b]">Trust Score</p>
              <p className="mt-1 text-2xl font-semibold">{Number(profile.trustScore ?? 0).toFixed(1)}</p>
            </div>
          </div>

          <div className="mt-4 rounded-xl border border-[#121212] bg-[#f5f5f5] p-4">
            <p className="text-sm text-[#4b4b4b]">Skills</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {(profile.skills ?? []).map((skill: string) => (
                <span key={skill} className="rounded-full border border-[#121212] px-2.5 py-1 text-xs text-[#2a2a2a]">
                  {skill}
                </span>
              ))}
            </div>
          ...
          <div className="mt-4 rounded-xl border border-[#121212] bg-[#f5f5f5] p-4">
            <p className="text-sm text-[#4b4b4b]">Experience</p>
            <p className="mt-2 text-sm text-[#2a2a2a]">{profile.experience || "No summary added."}</p>
          </div>
        </Card>
      ) : null}
    </motion.section>
